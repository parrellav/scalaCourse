...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def iter(inputSet: TweetSet, sortedSet: TweetList): TweetList = {
      if(inputSet.isEmpty) sortedSet
      else {
        new Cons(inputSet.mostRetweeted, iter(inputSet.remove(inputSet.mostRetweeted), sortedSet))
      }
    }
    iter(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    val popular = combined.filter(p => p.retweets > elem.retweets)
